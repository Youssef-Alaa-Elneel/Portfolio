...
const particlesConfig = {
    density: 250,         // الكثافة (عدد النقط)
    dotSize: 6,           // حجم النقط بالبيكسل
    speed: 50,            // السرعة
    direction: 0,         // اتجاه الحركة (0 يعني نزول لتحت)
    hoverIntensity: 200,  // مدى تأثر النقط وتفاعلها مع الماوس
    linkDistance: 200,    // أقصى مسافة لربط النقط ببعضها بخطوط
    linkThickness: 1,     // سمك الخطوط اللي بتربط النقط
...
let updateParticleColors = null; // Exposed globally to update colors on theme toggle
...
function initParticles() {
    ...
    let builtW = 0;
    let builtH = 0;
    let ptr = { x: -10000, y: -10000 };
    
    // Determine initial color based on data-theme attribute on html
    const htmlTheme = document.documentElement.getAttribute("data-theme") || "dark";
    let currentBase = htmlTheme === 'light' ? particlesConfig.light.baseColor : particlesConfig.dark.baseColor;
    let currentAccent = htmlTheme === 'light' ? particlesConfig.light.accentColor : particlesConfig.dark.accentColor;

    updateParticleColors = (theme) => {
        if (theme === 'light') {
            currentBase = particlesConfig.light.baseColor;
            currentAccent = particlesConfig.light.accentColor;
        } else {
            currentBase = particlesConfig.dark.baseColor;
            currentAccent = particlesConfig.dark.accentColor;
        }
    };
    ...
    const render = (now) => {
        ...
        const sp = particlesConfig.speed / 50;
        ...
        let targetDensity = Math.round(Math.max(10, Math.min(400, particlesConfig.density)));
        ...
        const hv = particlesConfig.hoverIntensity / 100;
        const reach = 180 * (hv > 0 ? 1 : 0);
        const LINKD = particlesConfig.linkDistance;
        const LINKW = particlesConfig.linkThickness;

        const th = (particlesConfig.direction * Math.PI) / 180;
        ...
        const cb = parseColor(currentBase, [0.612, 0.639, 0.686]);
        const ca = parseColor(currentAccent, [0.376, 0.647, 0.98]);
        ...
        if (nCount > 0) {
            ...
            gl.uniform1f(u(dotProg, "uDot"), particlesConfig.dotSize);
    ...
    window.addEventListener("pointerenter", track);
    window.addEventListener("pointerleave", onLeave);

    raf = requestAnimationFrame(render);
}

// Start particles automatically
